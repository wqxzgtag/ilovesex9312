document.addEventListener("DOMContentLoaded", function() {
    ...
    const currentTrackNumber = (currentDayOfWeek % trackList.length) + 1;
    const sourceElement = document.querySelector(`source[data-track-number="${currentTrackNumber}"]`);

    if (sourceElement) {
        audio.src = sourceElement.src;
        songTitle.innerText = trackList[currentTrackNumber - 1].innerText;
        audio.play();
    } else {
        console.error("No matching track found for the current day.");
    }
